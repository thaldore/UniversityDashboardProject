...
const EditIndicatorPage = () => {
    ...
    const loadInitialData = async () => {
        try {
            ...
            // Paralel olarak indicator ve departments yükle
            const [indicatorData, departmentsData] = await Promise.all([
                indicatorService.getIndicatorById(id),
                indicatorService.getDepartments()
            ]);

            if (!indicatorData) {
                setError('Gösterge bulunamadı.');
                return;
            }
            ...
            setFormData({
                ...
                departmentId: indicatorData.departmentId || '',
                assignedUserId: indicatorData.assignedUserId || '',
                notificationUserId: indicatorData.notificationUserId || '',
                dataType: indicatorData.dataType || IndicatorDataType.Number,
                periodStartDate: indicatorData.periodStartDate ? indicatorData.periodStartDate.split('T')[0] : '',
                periodType: indicatorData.periodType || PeriodType.Year,
                notificationPeriod: indicatorData.notificationPeriod || 7,
                isActive: indicatorData.isActive ?? true,
                isAutomatic: indicatorData.isAutomatic ?? false,
                rootValues: indicatorData.rootValues?.length > 0 
                    ? indicatorData.rootValues.map(rv => ({
                        rootValue: rv.rootValue || '',
                        description: rv.description || '',
                        sortOrder: rv.sortOrder || 1
                    }))
                    : [{ rootValue: '', description: '', sortOrder: 1 }],
                historicalData: indicatorData.historicalData?.length > 0
                    ? indicatorData.historicalData.slice(0, 2).map(hd => ({
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const submitData = {
                ...
                periodStartDate: formData.periodStartDate || null,
                rootValues: formData.rootValues.filter(rv => rv.rootValue.trim()),
                historicalData: formData.historicalData.filter(hd => hd.value && hd.value.toString().trim())
                    .map(hd => ({ ...hd, value: parseFloat(hd.value) }))
            };

            await indicatorService.updateIndicator(id, submitData);
            navigate('/indicators', { 
                state: { 
                    message: 'Gösterge başarıyla güncellendi!', 
                    type: 'success' 
                } 
            });
        } catch (error) {
            setError('Gösterge güncellenirken hata oluştu.');
        ...
        <div className="indicator-form-page">
            <div className="page-header">
                <div className="header-content">
                    <h1 className="page-title">Göstergeyi Düzenle</h1>
                    <p className="page-subtitle">Gösterge bilgilerini güncelleyin</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="indicator-form">
                ...
                <div className="form-actions">
                    ...
                    <button
                        ...
                        disabled={loading}
                    >
                        {loading ? 'Güncelleniyor...' : 'Göstergeyi Güncelle'}
                    </button>
                </div>
            </form>
        </div>
